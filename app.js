...
function renderDashboard(){
  ...
  const k_noshow = todayAp.filter(a=>a.status==='no_show').length;
  const k_espera = todayAp.filter(a=>['checked_in','triage'].includes(a.status)).length;
  const k_atendidos = todayAp.filter(a=>a.status==='completed').length;
  ...
  tb.querySelectorAll('[data-checkin]').forEach(btn=> btn.addEventListener('click', ()=> changeApptStatus(btn.dataset.checkin, 'checked_in')));
  tb.querySelectorAll('[data-cancel]').forEach(btn=> btn.addEventListener('click', ()=> changeApptStatus(btn.dataset.cancel, 'no_show')));
...
document.getElementById('btnGuardarCita')?.addEventListener('click', ()=>{
  ...
  const ap = { id: uid('ap'), datetime: new Date(document.getElementById('citaFecha').value).toISOString(), doctorId: document.getElementById('citaDoctor').value, patientId: pid, service: document.getElementById('citaServicio').value || 'Consulta', status:'scheduled', priority:'Media' };
...
function prepareConsulta(apptId){
  const db = loadDB();
  let ap = apptId ? db.appointments.find(a=>a.id===apptId) : db.appointments.find(a=>a.status==='in_consult');
...
document.getElementById('btnFirmarCerrar')?.addEventListener('click', ()=>{
  ...
  ap.status = 'completed'; saveDB(db); toast('Consulta firmada'); confetti(); renderDashboard();
});
...
function boostDemoData(){
  ...
  for(let i=0;i<30;i++){
    ...
    db.appointments.push({
      ...
      status:['scheduled','checked_in','triage','in_consult','completed','no_show'][i%6],
...
// ==== DESKTOP SAFEGUARD ====
// Si entramos en desktop con el drawer abierto por algún motivo, ciérralo.
