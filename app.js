...
function loadDB(){
  ...
  if(!db){ db = JSON.stringify(seedDB()); localStorage.setItem(DB_KEY, db); }
...
function seedDB(){
  ...
  const times = ['08:30','09:15','10:00','11:30'];
  const services = ['Consulta general','Control','Ecografía','Consulta'];
  for(let i=0;i<4;i++){
    const t = times[i].split(':'), dt = new Date();
    dt.setHours(parseInt(t[0]), parseInt(t[1]),0,0);
    db.appointments.push({ id:uid('ap'), datetime:dt.toISOString(), doctorId:db.doctors[i%db.doctors.length].id, patientId:db.patients[i%db.patients.length].id, service:services[i], status:'Programado', priority:'Media' });
  }
  return db;
}
function getSession(){ const s=localStorage.getItem(SESS_KEY); return s? JSON.parse(s): null }
function setSession(u){ localStorage.setItem(SESS_KEY, JSON.stringify({id:u.id, name:u.name, email:u.email, role:u.role})) }
...
document.querySelectorAll('.menu .menu-item').forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    closeDrawer(); // autocierre en móvil
  });
});
...
(function initAuth(){
  const s = getSession();
  if(s){ document.getElementById('userBadge').textContent = s.name.split(' ').map(w=>w[0]).slice(0,2).join('').toUpperCase(); app.setAttribute('data-auth','true'); showView('dashboard'); applyRole(s.role||'admin'); renderDashboard(); }
  else { showView('login'); }
})();
...
document.getElementById('toggleDark').addEventListener('click', ()=>{ const isDark = root.classList.toggle('dark'); toast(isDark ? 'Tema oscuro' : 'Tema claro'); });
...
document.getElementById('btnDoLogin')?.addEventListener('click', ()=>{
  ...
  if(user){ setSession(user); document.getElementById('userBadge').textContent = user.name.split(' ').map(w=>w[0]).slice(0,2).join('').toUpperCase(); showView('dashboard'); applyRole(user.role||'admin'); renderDashboard(); toast('Bienvenido'); }
...
document.getElementById('btnGoForgot')?.addEventListener('click', ()=> showView('forgot'));
document.getElementById('btnSendReset')?.addEventListener('click', ()=>{ toast('Si el email existe, recibirás un enlace (demo)'); showView('login'); });
document.getElementById('btnCreateUser')?.addEventListener('click', ()=>{
  ...
  const usr = {id:uid('usr'), name, email, pass, role}; db.users.push(usr); saveDB(db); toast('Usuario creado. Inicia sesión.'); showView('login');
});
document.getElementById('btnLogout')?.addEventListener('click', ()=>{ clearSession(); toast('Sesión cerrada'); showView('login'); });
...
function renderDashboard(){
  ...
  const k_citas = todayAp.length;
  const k_noshow = todayAp.filter(a=>a.status==='No se Presento').length;
  const k_espera = todayAp.filter(a=>['Registrado','Triage'].includes(a.status)).length;
  const k_atendidos = todayAp.filter(a=>a.status==='Completado').length;
  kpiCitas.textContent = k_citas; kpiNoShow.textContent = k_noshow; kpiEspera.textContent = k_espera; kpiAtendidos.textContent = k_atendidos;

  drawBarChart('chartEstados', ['Prog','Check','Triage','Cons','OK','NoShow'], [
    todayAp.filter(a=>a.status==='Programado').length,
    todayAp.filter(a=>a.status==='Registrado').length,
    todayAp.filter(a=>a.status==='Triage').length,
    todayAp.filter(a=>a.status==='En Consulta').length,
    todayAp.filter(a=>a.status==='Completado').length,
    todayAp.filter(a=>a.status==='No se Presento').length,
  ]);

  const week = ['L','M','X','J','V','S','D'];
  const trend = week.map(()=> 12 + Math.floor(Math.random()*8));
  drawLineChart('chartSemana', week, trend);

  const tb = document.getElementById('tbCitasHoy'); tb.innerHTML='';
  todayAp.sort((a,b)=> new Date(a.datetime)-new Date(b.datetime)).forEach(a=>{
    const patient = db.patients.find(p=>p.id===a.patientId)?.name || '—';
    const doctor = db.doctors.find(d=>d.id===a.doctorId)?.name || '—';
    ...
    tr.innerHTML = `<div>${hora}</div><div>${patient}</div><div>${doctor}</div><div>${a.service}</div><div>${badge(a.status)}</div>
                    <div><button class="btn btn-outline" data-checkin="${a.id}">Check-in</button>
                    <button class="btn" data-cancel="${a.id}">No se Presento</button></div>`;
    tb.appendChild(tr);
  });
  tb.querySelectorAll('[data-checkin]').forEach(btn=> btn.addEventListener('click', ()=> changeApptStatus(btn.dataset.checkin, 'Registrado')));
  tb.querySelectorAll('[data-cancel]').forEach(btn=> btn.addEventListener('click', ()=> changeApptStatus(btn.dataset.cancel, 'No se Presento')));

  document.getElementById('btnQuickAppt').onclick = ()=>{ document.querySelector('[data-target="agenda"]').click(); btnNuevaCita.click(); };
  document.getElementById('btnBoostData').onclick = boostDemoData;
}
...
function renderPacientes(){
  const db = loadDB(); const tb=document.getElementById('tbPacientes'); tb.innerHTML='';
  ...
  document.getElementById('btnNewPatientTop').onclick = ()=> showView('newPatient');
  tb.querySelectorAll('[data-ver]').forEach(btn=> btn.addEventListener('click', ()=>{ const db2=loadDB(); alert(JSON.stringify(db2.patients.find(x=>x.id===btn.dataset.ver), null, 2)) }));
}
document.getElementById('btnSavePatient')?.addEventListener('click', ()=>{
  const db = loadDB();
  const p = { id: uid('pat'), document: pDoc.value.trim(), dob: pDob.value, name: pName.value.trim(), phone:pPhone.value.trim(), address:pAddr.value.trim(), allergies:(pAllergies.value||'').split(',').map(s=>s.trim()).filter(Boolean), coverage: pCoverage.value.trim() };
...
function renderAgenda(){
  const db = loadDB();
  const cols = document.getElementById('agendaCols'); cols.innerHTML='';
  const sel = document.getElementById('agDoctor'); sel.innerHTML = '<option value="">Todos los médicos</option>' + db.doctors.map(d=>`<option value="${d.id}">${d.name} — ${d.specialty}</option>`).join('');
  db.doctors.forEach(d => {
    ...
    list.forEach(a => {
      ...
      const slot = document.createElement('div'); slot.className='slot' + (a.status!=='Programado' ? ' booked':'');
      const patient = db.patients.find(p=>p.id===a.patientId)?.name || '—';
...
document.getElementById('btnNuevaCita')?.addEventListener('click', ()=>{
  const db = loadDB(); const sel = document.getElementById('citaDoctor'); sel.innerHTML = db.doctors.map(d=>`<option value="${d.id}">${d.name}</option>`).join('');
  document.getElementById('citaFecha').value = new Date(Date.now()+3600000).toISOString().slice(0,16);
  document.getElementById('modalCita').classList.remove('hidden');
});
document.getElementById('closeModalCita')?.addEventListener('click', ()=> document.getElementById('modalCita').classList.add('hidden'));
document.getElementById('cancelModalCita')?.addEventListener('click', ()=> document.getElementById('modalCita').classList.add('hidden'));
document.getElementById('btnPickPaciente')?.addEventListener('click', ()=>{
  const q = document.getElementById('citaPaciente').value.toLowerCase();
  const db = loadDB(); const match = db.patients.find(p=> (p.name||'').toLowerCase().includes(q) || (p.document||'').toLowerCase().includes(q));
  if(match){ document.getElementById('citaPaciente').value = `${match.name} (${match.document})`; document.getElementById('citaPaciente').dataset.pid = match.id; toast('Paciente seleccionado'); }
  else toast('No encontrado');
});
document.getElementById('btnGuardarCita')?.addEventListener('click', ()=>{
  const db = loadDB();
  const pid = document.getElementById('citaPaciente').dataset.pid; if(!pid){ toast('Selecciona paciente'); return; }
  const ap = { id: uid('ap'), datetime: new Date(document.getElementById('citaFecha').value).toISOString(), doctorId: document.getElementById('citaDoctor').value, patientId: pid, service: document.getElementById('citaServicio').value || 'Consulta', status:'Programado', priority:'Media' };
  db.appointments.push(ap); saveDB(db); toast('Cita creada'); document.getElementById('modalCita').classList.add('hidden'); renderDashboard(); renderAgenda();
});

// ===== Sala de espera & Triage =====
let TriageTarget = null;
function renderSalaEspera(){
  const db = loadDB(); const tb = document.getElementById('tbEspera'); tb.innerHTML='';
  const list = db.appointments.filter(a => ['Registrado','Triage','En Consulta'].includes(a.status)).sort((a,b)=> new Date(a.datetime)-new Date(b.datetime));
  list.forEach(a=>{
  ...
  tb.querySelectorAll('[data-consulta]').forEach(b => b.addEventListener('click', ()=> { changeApptStatus(b.dataset.consulta, 'En Consulta'); prepareConsulta(b.dataset.consulta); showView('consulta'); }));
  tb.querySelectorAll('[data-complete]').forEach(b => b.addEventListener('click', ()=> { changeApptStatus(b.dataset.complete, 'Completado'); }));
}
document.getElementById('btnEnviarConsulta')?.addEventListener('click', ()=>{
  ...
  ap.status = 'Triage';
  ap.priority = document.getElementById('tPriority').value;
  ...
  saveDB(db); toast('Triage guardado y enviado a consulta'); changeApptStatus(ap.id, 'En Consulta'); prepareConsulta(ap.id); showView('consulta');
});
...
function prepareConsulta(apptId){
  const db = loadDB();
  let ap = apptId ? db.appointments.find(a=>a.id===apptId) : db.appointments.find(a=>a.status==='En Consulta');
  if(!ap){ document.getElementById('encabezadoEncuentro').textContent = 'No hay paciente en consulta'; return; }
  currentApptId = ap.id;
  const p = db.patients.find(x=>x.id===ap.patientId);
  const d = db.doctors.find(x=>x.id===ap.doctorId);
  document.getElementById('encabezadoEncuentro').textContent = `${p.name} • ${d.name} • ${new Date(ap.datetime).toLocaleTimeString([], {hour:'2-digit',minute:'2-digit'})} • ${ap.status}`;
  let enc = db.encounters.find(e=>e.appointmentId===ap.id);
  ...
  const vitals = enc.vitals || {}; const pv = document.getElementById('panelVitals'); pv.innerHTML='';
  Object.entries(vitals).forEach(([k,v])=>{ if(v) { const s=document.createElement('span'); s.className='chip'; s.textContent = `${k} ${v}`; pv.appendChild(s)} });
  const ch = document.getElementById('chipsDx'); ch.innerHTML=''; (enc.dx||[]).forEach(code=>{ const s=document.createElement('span'); s.className='chip'; s.textContent=code; ch.appendChild(s)});
}
document.getElementById('btnAddDx')?.addEventListener('click', ()=>{
  if(!currentApptId) return;
  const db = loadDB(); const enc = db.encounters.find(e=>e.appointmentId===currentApptId);
  const code = dxInput.value.trim(); if(!code) return;
  enc.dx = enc.dx || []; enc.dx.push(code); saveDB(db); prepareConsulta(currentApptId);
});
document.getElementById('btnFirmarCerrar')?.addEventListener('click', ()=>{
  if(!currentApptId) return;
  const db = loadDB(); const enc = db.encounters.find(e=>e.appointmentId===currentApptId); const ap = db.appointments.find(a=>a.id===currentApptId);
  enc.notes = {S:soapS.value,O:soapO.value,A:soapA.value,P:soapP.value}; enc.signed = true;
  ap.status = 'Completado'; saveDB(db); toast('Consulta firmada'); confetti(); renderDashboard();
});
document.getElementById('btnImprimirNota')?.addEventListener('click', ()=>{
  if(!currentApptId) return;
  const db = loadDB(); const enc = db.encounters.find(e=>e.appointmentId===currentApptId); const ap = db.appointments.find(a=>a.id===currentApptId);
  const p = db.patients.find(x=>x.id===ap.patientId); const d = db.doctors.find(x=>x.id===ap.doctorId);
...
function renderOrdenes(){
  const db = loadDB(); const tb = document.getElementById('tbOrdenes'); tb.innerHTML='';
  db.orders.forEach(o=>{
...
function openResultado(id){
  ...
  selectedOrderId = id;
  const html = `<p><strong>Tipo:</strong> ${o.type}</p><p><strong>Proveedor:</strong> ${o.provider||'-'}</p><p><strong>Notas:</strong> ${o.notes||'-'}</p><hr/><p><strong>Resultado:</strong> ${o.result?.summary||'—'}</p>`;
  document.getElementById('resContenido').innerHTML = html;
  document.getElementById('modalResultado').classList.remove('hidden');
}
document.getElementById('closeModalRes')?.addEventListener('click', ()=> document.getElementById('modalResultado').classList.add('hidden'));
document.getElementById('btnMarcarEntregado')?.addEventListener('click', ()=>{
  if(!selectedOrderId) return;
  const db = loadDB(); const o = db.orders.find(x=>x.id===selectedOrderId); o.status='delivered'; saveDB(db);
  toast('Marcado como entregado'); document.getElementById('modalResultado').classList.add('hidden'); renderOrdenes();
});
document.getElementById('btnImprimirRes')?.addEventListener('click', ()=>{
  if(!selectedOrderId) return;
  const db = loadDB(); const o = db.orders.find(x=>x.id===selectedOrderId);
  const w = window.open('', '_blank'); w.document.write(`
    <html><head><title>Resultado</title><style>
      ...
      <h2>Resultado de ${o.type}</h2><hr/>
      <div class="box">${o.result?.summary||'—'}</div>
      <script>window.onload=()=>window.print()</script>
    </body></html>
  `); w.document.close();
});
...
function renderReceta(){
  const tb = document.getElementById('tbReceta'); tb.innerHTML='';
  addMedRow('Paracetamol','500 mg','c/8h','5 días','Tomar con agua');
}
function addMedRow(a='',d='',f='',du='',i=''){
  const tb = document.getElementById('tbReceta');
  const tr = document.createElement('div'); tr.className='tr'; tr.style.gridTemplateColumns='1fr 140px 140px 140px 1fr 80px';
...
document.getElementById('btnFirmarReceta')?.addEventListener('click', ()=>{ toast('Receta firmada (demo)'); });
document.getElementById('btnImprimirReceta')?.addEventListener('click', ()=>{
  const rows = Array.from(document.querySelectorAll('#tbReceta .tr')).map(tr => Array.from(tr.querySelectorAll('input')).map(i=>i.value));
  const w = window.open('', '_blank'); w.document.write(`
...
function renderFactura(){
  const tb = document.getElementById('tbFactura'); tb.innerHTML='';
  const items = [ {c:'Consulta general', q:1, p:400}, {c:'Procedimiento', q:1, p:250} ];
  let total = 0;
  items.forEach(it=>{ const st = it.q*it.p; total += st; const tr=document.createElement('div'); tr.className='tr'; tr.style.gridTemplateColumns='2fr 120px 140px 140px'; tr.innerHTML=`<div>${it.c}</div><div>${it.q}</div><div>C$ ${it.p}</div><div>C$ ${st}</div>`; tb.appendChild(tr) });
  document.getElementById('factTotal').textContent = 'C$ ' + total;
}
document.getElementById('btnRegistrarPago')?.addEventListener('click', ()=> { toast('Pago registrado (demo)'); confetti(); });

// ===== Branding / Tema =====
document.getElementById('btnApplyTheme')?.addEventListener('click', ()=>{
  const p = document.getElementById('cPrimary').value;
  const s = document.getElementById('cSecondary').value;
  const st = document.createElement('style');
  st.textContent = `:root{--primary:${p};--secondary:${s}}`;
  document.head.appendChild(st);
  toast('Tema aplicado');
});
document.getElementById('btnThemeReset')?.addEventListener('click', ()=> location.reload());

// ===== Seed & Boost Data =====
document.getElementById('btnSeed')?.addEventListener('click', ()=>{ localStorage.removeItem(DB_KEY); loadDB(); toast('Base re-sembrada'); renderDashboard(); });
function boostDemoData(){
  ...
  for(let i=0;i<30;i++){
    ...
    db.appointments.push({
      ...
      patientId:db.patients[(i%db.patients.length)].id,
      service:['Consulta','Control','Ecografía'][i%3],
      status:['Programado','Registrado','Triage','En Consulta','Completado','No se Presento'][i%6],
      priority:['Baja','Media','Alta'][i%3]
    });
  }
  saveDB(db); toast('Datos demo generados'); renderDashboard(); renderAgenda(); renderPacientes();
}
document.getElementById('btnBoostData2')?.addEventListener('click', boostDemoData);
document.getElementById('btnBoostData')?.addEventListener('click', boostDemoData);
...
async function runDemo(){
  ...
  const db = loadDB();
  const pid = db.patients[0].id, did = db.doctors[0].id;
  const dt = new Date(); dt.setHours(new Date().getHours()+1,0,0,0);
  db.appointments.push({id:uid('ap'), datetime:dt.toISOString(), doctorId:did, patientId:pid, service:'Consulta', status:'Programado', priority:'Media'});
  ...
  changeApptStatus(apId, 'Registrado'); await sleep(300);
  document.querySelector('[data-target="sala"]').click(); await sleep(300);
  TriageTarget = apId;
  tTA.value='120/80'; tFC.value='78'; tFR.value='14'; tTemp.value='37.0'; tSpO2.value='98%'; tIMC.value='24'; tObs.value='Sin hallazgos relevantes'; tPriority.value='Media';
  document.getElementById('btnEnviarConsulta').click(); await sleep(600);
  dxInput.value='J06.9'; document.getElementById('btnAddDx').click();
  soapS.value='Paciente con odinofagia leve';
  soapO.value='TA 120/80, Temp 37, orofaringe congestiva';
  soapA.value='Faringitis aguda probable';
  soapP.value='Hidratación, paracetamol, reposo'; await sleep(400);
  document.getElementById('btnCrearOrden').click(); await sleep(400);
  const firstViewBtn = document.querySelector('#tbOrdenes .tr .btn'); if(firstViewBtn) firstViewBtn.click(); await sleep(400);
  document.getElementById('btnMarcarEntregado').click(); await sleep(400);
  document.querySelector('[data-target="recetas"]').click(); await sleep(300);
  document.getElementById('btnFirmarReceta').click(); await sleep(300);
  document.querySelector('[data-target="facturacion"]').click(); await sleep(300);
  document.getElementById('btnRegistrarPago').click(); await sleep(800);
  document.querySelector('[data-target="dashboard"]').click();
  toast('Demo finalizada ✨');
}

// ===== HOTFIX mobile: Demo CTA dentro de login =====
function ensureLoginDemoCTA() {
  ...
  if (isLogin && isSmall && !already) {
    ...
    if (src) {
      ...
      clone.addEventListener('click', runDemo);
    }
  }
}
